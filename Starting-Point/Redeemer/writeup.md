...
## 개요
이 문제는 기본 포트 스캔으로는 탐지되지 않는 서비스를 식별하고, Redis 서버에 인증 없이 접근하여 메모리에 저장된 데이터를 획득하는 과정을 다룬다. 핵심은 포트 스캔 범위 확장과 Redis의 보안 설정 문제를 이해하는 것이다.
...
별도의 인증 없이 접속이 가능하므로, 해당 Redis 인스턴스는 외부 접근에 대해 인증이 설정되어 있지 않음을 확인할 수 있다.
...
이를 통해 해당 데이터베이스에 실제 데이터가 존재함을 확인할 수 있다.
...
Redis는 기본적으로 인증이 설정되지 않은 상태로 실행될 수 있으며, 네트워크 설정에 따라 외부에서 접근이 가능하다. 특히 다음과 같은 조건이 결합될 경우 취약점이 발생한다.

- 인증 비활성화 (no password)
- 외부 인터페이스 바인딩 (0.0.0.0)
- 네트워크 접근 제어 미흡

이 경우 누구나 Redis 서버에 접속하여 내부 데이터를 조회하거나 수정할 수 있다.

또한 Redis는 메모리 기반 데이터베이스이기 때문에, 애플리케이션에서 사용하는 세션 정보나 인증 토큰 등이 평문 형태로 저장되어 있을 가능성이 높다.

---

## 7. 실제 환경에서의 위험성

이와 같은 설정은 단순 정보 노출을 넘어 심각한 보안 문제로 이어질 수 있다.

- 인증 정보 및 세션 탈취
- 내부 데이터베이스 정보 유출
- 서비스 계정 탈취

또한 Redis는 설정 변경이 가능하기 때문에 공격자가 다음과 같은 행위를 수행할 수 있다.
...
- 서비스 발견 후 직접 접근하여 동작을 확인하는 과정이 중요하다
- 단순한 명령어 실행이 아닌 서비스 구조 이해가 공격 성공의 핵심이다
